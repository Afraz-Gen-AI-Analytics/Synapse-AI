...
const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  // Intelligent Dashboard Routing: Handle Deep Links and Doorway Signals on startup
  useEffect(() => {
      const handleDeepLink = () => {
          // 1. Check for signal in sessionStorage (passed from Landing Page)
          const pendingIntent = sessionStorage.getItem('synapse_intent');
          
          // 2. Check for URL parameters (direct deep link to app)
          ...
          if (pendingIntent) {
              try {
                  ...
                  targetToolId = intent.toolId;
                  targetTopic = intent.topic;
                  // Clear it so it doesn't persist forever
                  sessionStorage.removeItem('synapse_intent');
              } catch (e) {
                  console.error("Failed to parse intent from storage", e);
              }
          } else if (urlTool) {
              targetToolId = urlTool;
              targetTopic = urlTopic || '';
              // Clean URL to look nice
              window.history.replaceState({}, document.title, window.location.pathname);
          }

          if (targetToolId) {
              // Normalize the ID: Handle slug cases like 'social-post' -> 'SocialMediaPost'
              // Basic mapping for common SEO slugs
              const slugMap: {[key: string]: string} = {
                  'social-post': 'SocialMediaPost',
                  'email-generator': 'MarketingEmail',
                  'blog-writer': 'SEOContentStrategist',
                  'ad-creative': 'AIAdCreativeStudio',
                  'viral-video': 'ViralVideoBlueprint',
                  'image-editor': 'AIImageEditor'
              };
              
              const mappedId = slugMap[targetToolId] || targetToolId;
              
              // Find template by ID or Name (fuzzy match)
              const targetTemplate = templates.find(t => 
                  t.id.toString() === mappedId || 
                  ContentType[t.id] === mappedId ||
                  t.name.replace(/\s/g, '') === mappedId
              );

              if (targetTemplate) {
                  handleTemplateSelect(targetTemplate);
                  if (targetTopic) {
                      // Delay slightly to ensure component mount
                      setTimeout(() => {
                          setTopic(targetTopic!);
                          // If it's a social post tool and platform is specified in query/intent
                          if (targetTemplate.id === ContentType.SocialMediaPost) {
                              // Extract platform from topic or query if present?
                              // For now, just set topic.
                          }
                      }, 100);
                      addToast(`Configured ${targetTemplate.name} based on your selection.`, 'success');
                  }
              }
          }
      };
