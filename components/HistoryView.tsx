            ...
            <div className="p-6 flex-1 overflow-y-auto">
                {activeTab === 'tools' && (
                    <div className="space-y-4">
                        {isToolHistoryLoading ? <HistorySkeleton /> : toolHistory.length > 0 ? toolHistory.map(item => {
                            const isImageItem = item.templateName === 'AI Ad Creative' || item.templateName === 'AI Image Editor';
                            ...
                            <div key={item.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/70 flex items-start">
                                <div className="w-16 h-16 rounded-md mr-4 flex-shrink-0 bg-slate-700 flex items-center justify-center overflow-hidden">
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                    <span className="text-xs bg-fuchsia-500/10 text-fuchsia-300 font-semibold px-2 py-1 rounded-full">{item.templateName}</span>
                                    ...
                                    <div className="flex-shrink-0 ml-2 md:ml-4">
                                        <div className="flex items-center text-sm text-slate-400">
                                            ...
                                            {/* COPY BUTTON */}
                                            <button onClick={() => onCopy(item.content, item.templateName, item.topic)} className="flex items-center p-2 hover:text-white transition-colors" title={item.content.startsWith('data:image') || item.templateName.includes("Video") ? "Copy Prompt" : "Copy Content"}>
                                                <CopyIcon className="w-4 h-4 md:mr-1.5" />
                                                <span className="hidden md:inline">{item.content.startsWith('data:image') || item.templateName.includes("Video") ? "Copy Prompt" : "Copy"}</span>
                                            </button>
